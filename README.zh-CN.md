...
传给工具的文档会经 HTTPS 上传到 HumanPen 的 API（`api.humanpen.net`）处理；
结果写回你的磁盘，已处理文件在服务端保留约 7 天，供 `check_job` 和免费再降
一次找回。文档内容永远不会进入模型上下文。完整政策（收集什么、保留多久、
如何联系）见 <https://humanpen.net/legal/privacy>。
