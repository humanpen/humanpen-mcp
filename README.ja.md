...
ツールに渡した文書は HTTPS で HumanPen の API（`api.humanpen.net`）に
アップロードされて処理されます。結果はディスクに書き戻され、処理済み
ファイルは `check_job` と無料の再ヒューマナイズが参照できるよう約 7 日間
サーバー側に保持されます。文書の内容がモデルのコンテキストに入ることは
ありません。全文（収集内容・保持期間・連絡先）は
<https://humanpen.net/legal/privacy> を参照してください。
